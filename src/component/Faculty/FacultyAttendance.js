...
import { getSelectedSection,getCoursesForFaculty, getFacultyProfile, getSectionsForFaculty, getStudentAccountsForFaculty, getStudentProfilesForFaculty, getTimeTableForParticularFaculty, updateParticularStudentMarks, updateParticularStudentAttendance } from "../../store/FacultyActions/faculty-actions";
...
const FacultyAttendance = () => {
    ...
    const initializeModalData=(sid)=>{
        ...
        let name=stud.name;
        let smarks=selectedSection?.attendance?.filter((s)=>s.studentId===sid)?.[0]?.subjects?.filter((sub)=>sub.subjectId===subjectId)?.[0]
    ...
    const marksChangeHandler=(e)=>{
        ...
        if(t.totalDays>0 && t.daysPresent>0)
            t.percentage=(t.daysPresent/t.totalDays)*100;
        else
            t.percentage=0;
    ...
    const updateStudentMarks=()=>{
        let data={
                ...
                totalDays:tempMarks.totalDays,
                daysPresent:tempMarks.daysPresent,
                sectionId:sectionId,
        }
        console.log(data)
        dispatch(updateParticularStudentAttendance(data))
    }
        ...
        {(!(_.isEmpty(timeTable))) &&
            <>
                <section className="d-flex justify-content-center align-items-center " style={{ backgroundColor: "#66ff99" }}>
                    <div className="">
                        <div className="h1 fw-bolder fs-1 text-center my-5">Give Attendance</div>
                    </div>
                </section>
                <div className="my-3">
                        ...
                        <table class="table align-middle text-center">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col">Att. Percentage</th>
                            ...
                            <tbody>
                                {studentAccounts.map((myStud, index) => {
                                    ...
                                    let studPercentage=selectedSection?.attendance?.filter((s)=>s.studentId===myStud._id)?.[0]?.subjects?.filter((sub)=>sub.subjectId===subjectId)?.[0]?.percentage || 'Nil';
...
{/* MOdal */}
                ...
                <div class="modal-content">
                    ...
                    <h1 class="modal-title fs-5" id="exampleModalLabel">Student Attendance Data</h1>
                    ...
                    <input type="number" className="form-control" name="daysPresent" value={tempMarks?.daysPresent} onChange={(e)=>marksChangeHandler(e)}/>

                    <label>Total No. of Days</label>
                    <input type="number" className="form-control" name="totalDays" value={tempMarks?.totalDays} onChange={(e)=>marksChangeHandler(e)}/>
